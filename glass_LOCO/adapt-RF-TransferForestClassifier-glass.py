...
for i, cluster_id in enumerate(range(0,ncluster)):
    ...
    #     verbose=0 , n_estimators=10, Xt=Xt.iloc[:10], yt=yt[:10], random_state=0)
    # adapt_model=TransferTreeClassifier(src_only)
    adapt_model=TransferForestClassifier(estimator=src_only, Xt=Xt.iloc[:10], yt=yt[:10], algo='', 
            bootstrap=False, verbose=0, random_state=0)

    adapt_model.fit_estimator(X=Xt.iloc[:10], y=yt[:10])

    # Get the score on target data
    adapt_score = adapt_model.score(Xt[10:], yt[10:]) #R2
