...
for i, cluster_id in enumerate(range(0,ncluster)):
    ...
    #print(y_pred, "***0.1)")
    #print(yt)
    # score = balanced_accuracy(yt,y_pred)
    ...
    adapt_model=RegularTransferLC(src_only, lambda_=10.)

    n = len(target_id_list)//2
    adapt_model.fit(Xt.iloc[:n], yt[:n])
    Xt=Xt.iloc[n:]
    yt=yt[n:]
    ...
    # adapt_score = balanced_accuracy(yt,y_pred)
