...
for i, cluster_id in enumerate(range(0,ncluster)):
    ...
    print("src_only accuracy: ", score, end="")
    ...
    n_finetune_samples=3
    adapt_model=LinInt(RandomForestClassifier(n_estimators=100, max_depth=10), 
        Xt=Xt.iloc[:n_finetune_samples], yt=yt[:n_finetune_samples], prop=0.5, verbose=0,            
              random_state=0)

    #print(Xt.iloc[[1,-1]])

    adapt_model.fit(Xs, ys)
    Xt=Xt.iloc[n_finetune_samples:]
    yt=yt[n_finetune_samples:]
    ...
    if adapt_score == score:
        ...
        average_improvement += w
    else:
        num_worse += 1
        w = -(score-adapt_score)/score
        average_worse += w
    print()
